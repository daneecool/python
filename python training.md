...
問題 41<br>
次のセットを取り出し要素を１⾏ずつ画面表示してください。<br>
...
# ミュータブル（変更可能）の例
my_list = [1, 2, 3]
my_list.append(4)  # O OK: [1, 2, 3, 4]

my_string = "hello"
# my_string[0] = "H"  # X エラー: 文字列は変更不可

# イテラブル（反復可能）の例
for char in "Python":  # O OK: P, y, t, h, o, n
    print(char)

# for num in 123:  # X エラー: 整数は反復不可

# シーケンス型（順序あり）の例
my_list = ["a", "b", "c"]
print(my_list[1])  # O OK: "b"

my_set = {1, 2, 3}
# print(my_set[0])  # X エラー: セットはインデックスアクセス不可
```
...
# クラスの基本
問題 1<br>
次の文章の空欄に⼊れるべき語句を、選択肢から選んでください。<br>
 Pythonは[（1）]指向型の言語といわれ、クラスは[（1）]の属性や機
能を定義したものです。<br>
 クラスの定義の中で[（1）]の持つ情報は[（2）]に持たせることがで
き、機能は[（3）]に持たせることができます。<br>
 インスタンスが生成されるときに⾃動的に呼び出されるメソッドを [
（4）]またはコンストラクタと呼びます。<br>
【選択肢】
・クラス変数 ・インスタンス変数 ・関数
・オブジェクト ・メソッド ・初期化メソッド
...
インスタンスが生成されるときに⾃動的に呼び出されるメソッドを**初期化メソッド**またはコンストラクタと呼びます。

**解説:**
- **オブジェクト指向**: データと処理を一つにまとめるプログラミング手法
- **インスタンス変数**: 各オブジェクトが個別に持つデータ（`self.name`, `self.age`など）
- **メソッド**: クラス内で定義される関数。オブジェクトの操作を行う
- **初期化メソッド**: `__init__()`メソッド。オブジェクト作成時に自動実行
- **コンストラクタ**: 初期化メソッドの別名。オブジェクトを構築する
...
問題 2<br>
個人の情報を表すPersonクラスを作成し、以下の要件を満たすコードを完成させてください。<br>

**要件:**
- `Person`クラスに`__init__`メソッドで`name`（名前）と`age`（年齢）を設定
- 空欄（A）に適切なメソッドを追加
- 空欄（B）でそのメソッドを呼び出し、戻り値がある場合は出力する
...
# (B) 追加したメソッドを呼び出す
print(a.introduce())  # 自己紹介を出力
print(b.introduce())  # 自己紹介を出力

# 年齢比較の例
if a.is_older_than(b):
    print(f"{a.name}は{b.name}より年上です")
else:
    print(f"{a.name}は{b.name}より年下です")
```
...
**解説:**
- **クラス定義**: `class ClassName:` でクラスを定義
- **コンストラクタ**: `__init__(self, ...)` で初期化処理を行う
- **インスタンス変数**: `self.name`, `self.age` でオブジェクトの属性を保存
- **メソッド**: クラス内の関数。第一引数は慣例的に`self`
- **インスタンス化**: `Person("名前", 年齢)` でオブジェクトを作成
- **メソッド呼び出し**: `オブジェクト.メソッド名()` で実行
...
問題 2-1<br>
次のPersonクラスの空欄（A）に適切なメソッドを追加し、空欄（B）でそのメソッドを呼び出してください。戻り値がある場合は出力するようにしてください。
...
**解説:**
- **空欄（A）**: クラス内にメソッドを定義。第一引数は必ず`self`
- **空欄（B）**: `オブジェクト.メソッド名()` でメソッドを呼び出し
- **`print_info()`**: 戻り値なし（None）のメソッド。直接情報を出力
- **`get_age()`**: 戻り値ありのメソッド。値を返すので`print()`で出力
- **`self`**: そのオブジェクト自身を参照。インスタンス変数にアクセス可能
...
class Person:
    ...
    def compare_age(self, other_person):
        """他のPersonオブジェクトと年齢を比較"""
        if self.age > other_person.age:
            return f"{self.name}は{other_person.name}より年上です"
        elif self.age < other_person.age:
            return f"{self.name}は{other_person.name}より年下です"
        else:
            return f"{self.name}と{other_person.name}は同い年です"

a = Person("高橋太郎", 19)
b = Person("小林花子", 18)

# (B) 追加したメソッドを呼び出す
print(f"高橋太郎が18歳を超えているか: {a.age_check(18)}")
print(f"小林花子が20歳を超えているか: {b.age_check(20)}")
print(f"高橋太郎は成人か: {a.is_adult()}")
print(f"小林花子は成人か: {b.is_adult()}")
print(a.compare_age(b))
```

**実行結果:**
```
高橋太郎が18歳を超えているか: True
小林花子が20歳を超えているか: False
高橋太郎は成人か: False
小林花子は成人か: False
高橋太郎は小林花子より年上です
```
...
**解説:**
- **`age_check(threshold)`**: 年齢が指定した閾値を超えているかを`True`/`False`で返す
- **`is_adult()`**: 成人判定（20歳以上）を行うメソッド
- **`compare_age(other_person)`**: 他のPersonオブジェクトと年齢を比較
- **戻り値の活用**: `print()`内で直接メソッドを呼び出して結果を表示
- **論理演算**: `>`, `>=`, `<`などの比較演算子でブール値を返す
...
class Person:
    ...
    # (A) 若い方の人の名前を返すメソッドを追加
    def get_younger_person_name(self, other_person):
        """2つのPersonオブジェクトの若い方の名前を返す"""
        if self.age < other_person.age:
            return self.name
        elif self.age > other_person.age:
            return other_person.name
        else:
            return self.name  # 同じ年齢の場合は自分の名前を返す
    
    # 年上の人の名前を返すメソッド
    def get_older_person_name(self, other_person):
        """2つのPersonオブジェクトの年上の名前を返す"""
        if self.age > other_person.age:
            return self.name
        elif self.age < other_person.age:
            return other_person.name
...
# (B) 追加したメソッドを呼び出す
print(f"若い方の人: {a.get_younger_person_name(b)}")
print(f"年上の人: {a.get_older_person_name(b)}")
print(f"年齢差: {a.age_difference(b)}歳")

# 同い年のケースもテスト
c = Person("田中次郎", 19)
print(f"高橋太郎と田中次郎の若い方: {a.get_younger_person_name(c)}")
```
...
**解説:**
- **`get_younger_person_name()`**: 2つのPersonオブジェクトを比較して若い方の名前を返す
- **条件分岐**: `if/elif/else`で年齢を比較し、適切な名前を返す
- **同い年の処理**: 年齢が同じ場合は呼び出し元（self）の名前を返す
- **絶対値計算**: `abs()`関数で年齢差の絶対値を求める
- **メソッドチェーン**: オブジェクト間でメソッドを呼び合う設計パターン
...
**解説:**
- **`get_area(self)`**: 引数は`self`のみ。面積を計算して返す
- **戻り値**: `self.width * self.height`で幅×高さを計算
- **計算例**: rec0は5×0=0、rec1は4×6=24
- **メソッド呼び出し**: `オブジェクト.get_area()`で面積を取得
...
問題 4
次の文章の空欄に⼊れるべき語句を答えてください。
 Pythonは [ (1) ] 指向型の言語であり、クラスは[ (1) ]の属性や機能
を定義したものです。
 'Hello'や'Python'といった文字列はstr型のオブジェクトですが、別の
表現をすると、strクラスの [ (2) ] である、ということができます。
 strクラスには、文字列に含まれる文字を⼩文字に変換するlowerとい
う[ (3) ]があります。
 a == bという式の値がTrueであったとき、aとbは [ (4) ] であるとい
い、a is bという式の値がTrueであったとき、aとbは [ (5) ] である
といいます。
...
strクラスには、文字列に含まれる文字を⼩文字に変換するlowerという**メソッド**があります。
a == bという式の値がTrueであったとき、aとbは**等しい**であるといい、a is bという式の値がTrueであったとき、aとbは**同一**であるといいます。

**解説:**
- **(1) オブジェクト指向**: データと処理をまとめるプログラミング手法
- **(2) インスタンス**: クラスから作られた具体的なオブジェクト
- **(3) メソッド**: クラス内で定義された関数
- **(4) 等しい (==)**: 値が同じかどうかを判定
- **(5) 同一 (is)**: 同じオブジェクトかどうかを判定（メモリ上の同一性）
...
A. クラス名の省略形
B. 他のクラスを継承するときのキーワード
C. インスタンス⾃身を参照する変数
D. プライベート変数を表す記号

**解答: C. インスタンス⾃身を参照する変数**
- selfはインスタンス自身を指す
...
**解答: A. Hello!**
- `h.greet()`メソッドが実行されて"Hello!"が出力される
...
**解答: B. 10**
- `s.x`はインスタンス変数で値が10なので、10が出力される
...
問題 10
インスタンス変数とクラス変数の違いを正しく説明している
ものを選択してください。
A. どちらも全インスタンスで共有される
B. クラス変数はインスタンスごとに異なる
C. インスタンス変数は各インスタンスで固有
D. クラス変数はselfを使⽤して定義する

**解答: C. インスタンス変数は各インスタンスで固有**
- インスタンス変数は各オブジェクトで独立、クラス変数は全インスタンスで共有
...
メソッド︓bark()
        【内容】画⾯上に「ワン︕」と出⼒する
         __init__(self,name)
        【内容】初期化メソッド、属性に設定する値を
        設定する

・作成したクラスをインスタンス化して、barkメソッド
コールする。
...
問題 13
・Rectangleクラスを作成し、area()で⻑⽅形の⾯積を返す。
属性︓width,height ※幅と高さ
メソッド︓area(self)
【内容】２つの属性を使⽤して⻑⽅形の⾯積を算出し、
戻り値で呼び出し元に返す。
__init__(self,width,height)
【内容】初期化メソッド、各属性に値を設定する。
・作成したクラスをインスタンス化して、areaメソッドを
コールし、取得した値を画⾯表示する。
...
問題 14
・Counterクラスを作成し、count_up()でカウントし、reset()で0に戻す。
属性︓count ※カウンター 初期値は0
メソッド︓count_up(self)
【内容】属性のcountを1増加する。
__init__(self)
【内容】初期化メソッド、属性であるcountを0初期化。
reset(self)
【内容】属性であるcountに対して0を設定。
・作成したクラスをインスタンス化する。
・count_upメソッドを２回実施して、属性であるcountを画⾯表示する。
・resetメソッドをコールし、再度countの値を画⾯表示する。
...
問題 15
・クラス変数count使⽤して、作成されたPersonクラスのインスタンス数
をカウントする。
クラス変数︓count 初期値は0でインスタンス作成毎に１づつ増加
属性︓name 名前
メソッド︓__init__(self,name)
【内容】初期化メソッド、属性に値を設定。
インスタンス毎にクラス変数を１増加。
・作成したクラスをインスタンス化する。
・もう一回Personクラスをインスタンス化する。
・クラス変数countを画⾯表示する。
...
class Person:
    ...
    def __init__(self, name):
        self.name = name
        Person.count += 1  # インスタンス作成毎に増加

# インスタンス化
person1 = Person("田中")
person2 = Person("佐藤")

print(f"作成されたインスタンス数: {Person.count}")
```
...
**解説:**
- クラス変数`count`は全インスタンスで共有される
- `__init__`でインスタンス作成毎に`Person.count`を増加
- `Person.count`でクラス変数にアクセス
...
**解説:**
- **A**: X Pythonに`private`キーワードは存在しない
- **B**: O 正しい。`__`（アンダースコア2つ）でプライベート変数を定義
- **C**: X `_`（アンダースコア1つ）は慣例的に内部使用を示すが、実際はアクセス可能
- **D**: X `:=`は代入式（walrus operator）でプライベート変数ではない

**補足:**
```python
class MyClass:
    def __init__(self):
        self.__private_var = 10    # プライベート変数
        self._internal_var = 20    # 内部使用（慣例）
        self.public_var = 30       # パブリック変数
```
...
問題 2
nameという属性をもつクラスPersonを定義し、nameは外部
からアクセスできないようにする。※カプセル化
・Personクラス を作成する。
属性︓name 
メソッド︓__init__(self,name)
【内容】初期化メソッド、属性に設定する値を
設定する
・作成したクラスをインスタンス化する。
・Personクラスの属性nameにアクセスしてエラーとなるこ
とを確認する。
...
class Person:
    def __init__(self, name):
        self.__name = name  # プライベート変数として定義

# インスタンス化
person = Person("田中太郎")

# プライベート変数への直接アクセスを試行（エラーになる）
try:
    print(person.__name)  # AttributeError が発生
except AttributeError as e:
    print(f"エラー: {e}")

# 正しくアクセスできない事を確認
print("プライベート変数への直接アクセスはできません")
```

**実行結果:**
```
エラー: 'Person' object has no attribute '__name'
プライベート変数への直接アクセスはできません
```

**解説:**
- `self.__name`でプライベート変数として定義
- 外部から`person.__name`でアクセスしようとするとAttributeErrorが発生
- Pythonは名前マングリング（name mangling）により`_Person__name`に変換
...
問題 3
問題2のPersonクラスにget_name()メソッドを追加して、属
性nameの値を取得可能とする。※カプセル化
追加メソッド︓get_name()
【内容】属性nameの中身を呼び出し元に
戻り値として戻す。
・作成したクラスをインスタンス化する。
・get_name()メソッドを呼び出し、属性nameの値を
取得し画⾯表示する
...
class Person:
    ...
    def get_name(self):
        """プライベート変数__nameの値を取得するgetter"""
        return self.__name

# インスタンス化
person = Person("田中太郎")

# getterメソッドを使用して名前を取得
...
**解説:**
- `get_name()`メソッドはgetterメソッドと呼ばれる
- プライベート変数`self.__name`にアクセスして値を返す
- 外部からはこのメソッドを通してのみプライベート変数にアクセス可能
- カプセル化の基本原則：データの隠蔽と制御されたアクセス
...
問題 4
問題3のPersonクラスにset_name()メソッドを追加して、属
性nameの値を設定可能とする。※カプセル化
追加メソッド︓set_name(self,name)
【内容】引数で設定された名前を属性name
に設定する
・作成したクラスをインスタンス化する。
・set_name()メソッドを呼び出し、属性nameに値を
設定する。
...
# 初期値を確認
print(f"初期名前: {person.get_name()}")

# setterメソッドを使用して名前を変更
person.set_name("佐藤花子")
print(f"変更後名前: {person.get_name()}")
```
...
**解説:**
- `set_name()`メソッドはsetterメソッドと呼ばれる
- プライベート変数`self.__name`に新しい値を設定
- getter/setterの組み合わせでプライベート変数への制御されたアクセスを実現
- データの整合性や妥当性チェックを追加可能
...
問題 5
・Personクラスに属性ageを追加しプライベート化する。
・属性ageに対してgetter/setterメソッドを作成する
※カプセル化
追加属性︓age 年齢情報
追加メソッド︓set_age(self,age)
【内容】引数で設定された年齢を属性age
に設定する
追加メソッド︓get_age(self,age)
【内容】属性ageの中身を呼び出し元に
戻り値として戻す。
・作成したクラスをインスタンス化する。
・set_age()メソッドを呼び出し、属性ageに値を設定する。
・get_age()メソッドを呼び出し設定したageの値を確認する。
...
class Person:
    ...
    def get_age(self):
        """プライベート変数__ageの値を取得するgetter"""
        return self.__age
    
    def set_age(self, age):
        """プライベート変数__ageの値を設定するsetter"""
        self.__age = age
...
**解説:**
- `__age`プライベート変数を追加
- `get_age()`と`set_age()`でageのgetter/setterを実装
- 外部からは専用メソッドを通してのみアクセス可能
...
問題 6
・Personクラスのset_age()メソッドでマイナスの年齢が設定されない
処理を追加する。
※カプセル化
変更メソッド︓set_age(self,age)
【内容】引数で設定された年齢を属性age
に設定する
引数のageの値がマイナスの場合、
「年齢は0以上である必要があります」と
...
# マイナスの年齢を設定（エラーメッセージが表示される）
person.set_age(-5)
print(f"設定後の年齢: {person.get_age()}歳")  # 変更されない

# 再度正常な年齢を設定
person.set_age(30)
print(f"最終年齢: {person.get_age()}歳")
```
...
**解説:**
- `set_age()`メソッドに妥当性チェックを追加
- マイナス値の場合はエラーメッセージを表示し、値を設定しない
- カプセル化により、データの整合性を保つことができる
- setterメソッドでデータ検証を行う典型的なパターン
...
問題 7
メソッドのオーバーライドとはなにか選択してください。
A. メソッドを削除すること
B. メソッドを複数定義すること
C. 継承元のメソッドを上書きすること
D. メソッドをプライベートにすること

**解答: C. 継承元のメソッドを上書きすること**

**解説:**
- **A**: X メソッドの削除ではない
- **B**: X メソッドオーバーロード（Pythonでは一般的でない）
- **C**: O 正しい。継承先のクラスで親クラスのメソッドを再定義すること
- **D**: X プライベート化とは関係ない
...
**解説:**
- コードに構文エラーがある：`class A` と `class B` の後にコロン（:）が不足
- 正しくは `class A:` と `class B:` であるべき
- Pythonでは class 定義にはコロンが必須
...
**（3）x.b()**
- エラー: `AttributeError: 'X' object has no attribute 'b'`
- Xクラスにb()メソッドは存在しない
...
- 出力: 
  ...
  [y]
  ```
- Yクラスの`__init__()`で`super().__init__()`によりXのコンストラクタも実行される
...
- 出力:
  ...
  [x.b]
  ```
- Yクラスのa()メソッドが実行され、`super().a()`でXクラスのa()も実行される

**（6）y.b()**
- エラー: `AttributeError: 'Y' object has no attribute 'b'`
- YクラスもXクラスもb()メソッドを持たない

**解説:**
- 継承関係：Y は X を継承
- `super()`で親クラスのメソッドを呼び出し
- メソッドのオーバーライド：YクラスのあるメソッドはXクラスのそれを上書き
...
**解説:**
- **A**: X `catch`はJavaなど他の言語。Pythonでは`except`
- **B**: O 正しい。Pythonの例外処理は`try/except`
- **C**: X `do`は例外処理のキーワードではない
- **D**: X これらは例外処理のキーワードではない
...
**解答: B. ZeroDivisionError**

**解説:**
- **A**: X TypeError - 型が間違っている時の例外
- **B**: O 正しい。ZeroDivisionError - ゼロで割った時の例外
- **C**: X ValueError - 値が不適切な時の例外
- **D**: X IndexError - インデックスが範囲外の時の例外
...
**解答: コードが提示されていないため判定不可**

**解説:**
問題文にコードが記載されていないため、正確な答えを判定できません。
一般的なゼロ除算の例外処理コードを想定すると：
...
**解説:**
- **A**: X except - 例外が発生した時に実行される
- **B**: X raise - 例外を意図的に発生させる
- **C**: X finally - 例外の有無に関わらず常に実行される
- **D**: O 正しい。else - tryブロックで例外が発生しなかった時のみ実行
...
問題 5<br>
finallyブロックの特徴を選択してください。

A. 例外がおきたときだけ実⾏される
B. 例外が起きなかったときだけ実⾏される
C. 常に実⾏される
D. 無視されることがある
...
**解説:**
- **A**: X それは`except`ブロックの特徴
- **B**: X それは`else`ブロックの特徴
- **C**: O 正しい。`finally`は例外の有無に関わらず必ず実行される
- **D**: X `finally`ブロックが無視されることはない
...
問題 6<br>
0で割ったときの例外処理を作成してください。<br>
例外を取得したときに、「0では割れません」と画⾯表示してください。
...
**解説:**
- `ZeroDivisionError`でゼロ除算例外をキャッチ
- 例外発生時に指定されたメッセージを表示
- プログラムは異常終了せず、例外処理後も継続
...
問題 7<br>
存在しないキーにディクショナリアクセスしたときの例外処理を作成してください。<br>
例外を取得したときに、「キーは存在しません」と画⾯表示してください。
data = {"name": "Tom"}
### "age"キーを取得しようとするが、存在しない
...
**解説:**
- `KeyError`で存在しないキーへのアクセス例外をキャッチ
- 辞書に存在しないキーにアクセスしようとすると`KeyError`が発生
- 安全な辞書アクセス方法として`get()`メソッドも使用可能

**別解（get()メソッド使用）:**
```python
age = data.get("age", "不明")
print(f"年齢: {age}")  # 年齢: 不明
```
...
問題 8<br>
リストでインデックスの範囲外アクセスしたときの例外処理
を作成してください。<br>
例外を取得したときに、「インデックスが範囲外です」と画
⾯表示してください。<br>
問題 8
リストでインデックスの範囲外アクセスしたときの例外処理を作成してください。
例外を取得したときに、「インデックスが範囲外です」と画⾯表示してください。
lst = [1, 2, 3]
### インデックス5を参照
...
**解説:**
- `IndexError`でインデックス範囲外例外をキャッチ
- リストのインデックスは0から始まり、lst[0], lst[1], lst[2]のみ有効
- インデックス5は存在しないため`IndexError`が発生
- 安全なアクセス方法として範囲チェックも可能
...
問題 9<br>
ファイルを開こうとするがファイルが存在しないときの例外処理を作成してください。
例外を取得したときに、「ファイルが⾒つかりません」と画⾯表示してください。
### "data.txt"という存在しないファイルを開く
...
**解説:**
- `FileNotFoundError`でファイルが存在しない例外をキャッチ
- `with`文を使用してファイルの自動クローズを保証
- ファイルが存在しない場合、プログラムは異常終了せずメッセージを表示
- ファイル操作では常に例外処理が推奨される
...
問題 10<br>
整数の⼊⼒処理（int(input(“整数を⼊⼒: ”))）を実施し、例
外が発生した場合は、「整数を⼊⼒してください」、正常な
場合は「正常です」と画⾯表示する例外処理を作成してくだ
さい。
...
**解説:**
- `ValueError`で文字列から整数への変換エラーをキャッチ
- 数字以外の文字列を入力した場合に例外が発生
- ユーザー入力の妥当性チェックに活用される典型的なパターン
...
問題 11<br>
次のコードは、リストに含まれる文字列を1つだけ出⼒します。どの文字列を出
⼒するかは、ユーザーが⼊⼒する値をインデックスに使⽤して決めます。⼊⼒し
...
```try〜except``` 構文を使⽤して、次のような例外処理を追加してください。
・ ⼊⼒した値が数字でない場合（```ValueError```が発生します）に「数字が⼊⼒されませんでした」と出⼒する
・ 数字であってもインデックスの範囲を超えている場合（```IndexError```が発生します）には、「範囲外の値が⼊⼒されました」と出⼒する
...
**解説:**
- 複数の例外を個別にキャッチして異なる処理を実行
- `ValueError`: `int(a)`で文字列を整数に変換できない場合
- `IndexError`: 整数は有効だがリストの範囲外の場合
- 例外の順序は重要（より具体的な例外を先に記述）
